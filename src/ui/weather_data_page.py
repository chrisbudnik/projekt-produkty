...
def expert_chat_component(location: str, weather_data: str):
    ...
    if prompt:
        ...
        full_prompt = WEATHER_LIFESTYLE_ASSISTANT.format(
            location=location, prompt=prompt, weather_data=weather_data
        )
...
def zmień_miasto(config_path="config/config.json"):
    ...
    if wpisane:
        ...
        try:
            location_list = geolocator.geocode(
                wpisane,
                exactly_one=False,
                addressdetails=True,
                limit=10,
        ...
        if not location_list:
            ...
            new_config = {
                "city_name": [
                    loc.raw["address"].get(
                        "city", loc.raw["address"].get("town", wpisane)
                    )
                ]
            }
            ...
            st.info(
                f"Znaleziono {len(location_list)} możliwych lokalizacji. Wybierz jedną:"
            )
            ...
            selected = st.selectbox(
                "Wybierz lokalizację:", [name for name, _ in options]
            )
...
def weather_forecast_page():
    ...
    with left:
        chart_type = st.radio(
            "Wybierz dane do wyświetlenia:", ["Temperatura", "Wiatr"], horizontal=True
        )
        ...
        if filtered_data.empty:
            ...
            fig.update_layout(
                xaxis_title="Godzina", height=500, margin=dict(t=30, b=30)
            )
    ...
    expert_chat_component(location=city_name, weather_data=data.to_csv(index=False))
